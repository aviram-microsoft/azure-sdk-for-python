...
class TestOncoPhenotype(AzureRecordedTestCase):
    ...
    def response_json(self, content) -> OncoPhenotypeResponse:
    ...
    @HealthDecisionSupportEnvPreparer()
    @recorded_by_proxy
    def test_invalid_clinical_type(self, healthdecisionsupport_endpoint, healthdecisionsupport_key):
        onco_client = OncoPhenotypeClient(healthdecisionsupport_endpoint, AzureKeyCredential(healthdecisionsupport_key))

        assert onco_client is not None

        data = self.load_json('onco_invalid_clinical_type_request.json')
